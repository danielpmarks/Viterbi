...
def viterbi_1(train, test):
    ...
    k = 1e-10
    word_counts, unique_words, unique_tags, tags, pair_counts = count_tags(
        train)
    ...
    predictions = viterbi(test, emission, transition, tags, k, word_counts)
    # print(predictions[0])
    return predictions
...
def count_tags(train):
    ...
    for sentence in train:
        for i in range(len(sentence) - 1):
            ...
            if word[1] in word_counts:
                if word[0] not in word_counts[word[1]]:
                    word_counts[word[1]][word[0]] = 1
                else:
                    word_counts[word[1]][word[0]] += 1
            else:
                word_counts[word[1]] = {}
            ...
            if word[1] not in pair_counts:
                ...
                if next_word[1] not in pair_counts[word[1]]:
                    ...
                    pair_counts[word[1]][next_word[1]] += 1
    # print(pair_counts)
    return word_counts, unique_words, unique_tags, tag_counts, pair_counts
...
def viterbi(test, emission, transition, tags, k, word_counts):
    '''
            ...
            if tag != 'START':
                ...
                if sentence[1] in emission[tag]:
                    ...
                    p_first_word = math.log(
                        k / (tags[tag] + k * (len(emission[tag]) + 1)))
